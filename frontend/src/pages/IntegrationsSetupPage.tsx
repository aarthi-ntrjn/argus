    ...
    body: (
      <>
        ...
        <p className="text-xs text-gray-400 mt-2">
          Use your tunnel URL for <Mono>{'<endpoint>'}</Mono> during local dev.
        </p>
      </>
    ),
  },
  {
    ...
    body: (
      <ol className="space-y-3 text-sm text-gray-600 list-decimal pl-4">
        <li>
          <strong>Team ID:</strong> Right-click the team name in Teams &rarr;{' '}
          <strong>Get link to team</strong>. The <Mono>groupId</Mono> parameter is your{' '}
          <strong>Team ID</strong>.
        </li>
        <li>
          <strong>Channel ID:</strong> Right-click the channel &rarr;{' '}
        ...
        <li>
          <strong>AAD Object ID:</strong> In the{' '}
          <ExternalA href="https://portal.azure.com">Azure portal</ExternalA>, go to{' '}
          <strong>Microsoft Entra ID &rarr; Users</strong>, click your account, and copy the{' '}
    ...
    body: (
      <>
        ...
        <ul className="space-y-1 text-sm text-gray-600 list-disc pl-4 mb-3">
          <li>
            <strong>Team ID</strong> and <strong>Channel ID</strong> from your Teams workspace
          </li>
          <li>
            <strong>Owner AAD Object ID</strong> (your Azure AD user object ID)
          </li>
          <li>
            <strong>Client ID</strong>, <strong>Client Secret</strong>, and{' '}
            <strong>Tenant ID</strong> from the bot create command
          </li>
        </ul>
        <p className="text-sm text-gray-600">
          Click <strong>Save</strong>, then restart Argus.
        </p>
      </>
    ),
  },
...
const TEAMS_TROUBLESHOOTING: { symptom: string; fix: React.ReactNode }[] = [
  {
    ...
    fix: (
      <>
        Check that <Mono>devtunnel host argus-tunnel</Mono> is running. The tunnel URL in the bot
        registration must match the one currently printed by devtunnel. Re-run{' '}
        <Mono>teams app create</Mono> if the URL changed.
      </>
    ),
  },
  {
    ...
    fix: (
      <>
        All six fields (Team ID, Channel ID, Owner AAD Object ID, Client ID, Client Secret, Tenant
        ID) must be saved in Settings. Restart Argus after saving.
      </>
    ),
  },
  {
    ...
    fix: (
      <>
        Client ID or Client Secret is wrong or expired. Re-run <Mono>teams app create</Mono> or
        regenerate the secret in the Azure App Registration.
      </>
    ),
  },
  {
    ...
    fix: (
      <>
        The client secret may have expired (default lifetime is 6 months). Regenerate it in the
        Azure portal under App Registrations &rarr; Certificates &amp; secrets, update it in
        Settings, and restart Argus.
      </>
    ),
  },
  {
    ...
    fix: (
      <>
        Make sure the bot was installed to the correct team via the install link printed by{' '}
        <Mono>teams app create</Mono>. The Team ID must match the team where the bot is installed.
      </>
    ),
  },
];
...
const SLACK_STEPS: SetupStep[] = [
  {
    ...
    body: (
      <ol className="space-y-1.5 text-sm text-gray-600 list-decimal pl-4">
        ...
        <li>
          Click <strong>Create New App</strong> &rarr; <strong>From scratch</strong>.
        </li>
        <li>
    ...
    body: (
      <>
        <ol className="space-y-1.5 text-sm text-gray-600 list-decimal pl-4 mb-3">
          ...
          <li>
            Scroll to <strong>Scopes &rarr; Bot Token Scopes</strong> and add these four:
          </li>
        </ol>
    ...
    body: (
      <>
        <ol className="space-y-1.5 text-sm text-gray-600 list-decimal pl-4 mb-3">
          ...
          <li>
            Create an App-level token: name it (e.g. <Mono>argus-socket</Mono>), add scope{' '}
            <Mono>connections:write</Mono>, click <strong>Generate</strong>.
          </li>
          <li>
    ...
    body: (
      <>
        <ol className="space-y-1.5 text-sm text-gray-600 list-decimal pl-4 mb-2">
          ...
          <li>
            Right-click the channel name &rarr; <strong>View channel details</strong>.
          </li>
          <li>
  ...
  },
  {
    ...
    body: (
      <>
        <p className="text-sm text-gray-600 mb-2">
          Open the Argus Settings dialog and go to the <strong>Slack</strong> section. Enter:
        </p>
        <ul className="space-y-1 text-sm text-gray-600 list-disc pl-4 mb-3">
          <li>
...
const SLACK_TROUBLESHOOTING: { symptom: string; fix: React.ReactNode }[] = [
  ...
  },
  {
    ...
    fix: (
      <>
        Confirm the App Token (<Mono>xapp-...</Mono>) is saved and Socket Mode is enabled in the
        Slack app settings. Check server logs for <Mono>[SlackListener]</Mono> errors.
      </>
    ),
  },
  {
    ...
    fix: (
      <>
        The Bot Token has been revoked. Re-install the app to your workspace (OAuth and Permissions
        &rarr; Install to Workspace) to generate a new token, update it in Settings, and restart
        Argus.
      </>
    ),
  },
  {
    ...
    fix: (
      <>
        The App Token may be expired or have insufficient scope. Delete it in{' '}
        <strong>Basic Information &rarr; App-Level Tokens</strong> and regenerate with the{' '}
        <Mono>connections:write</Mono> scope. Update in Settings and restart Argus.
      </>
    ),
  },
  {
    ...
    fix: (
      <>
        Verify the Channel ID in Settings. Use the channel ID (e.g. <Mono>C01234ABCDE</Mono>), not
        the channel name. Right-click the channel &rarr; <strong>View channel details</strong> to
        find it.
      </>
    ),
  },
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-gray-200 rounded-lg px-6 py-5">
        ...
        <p className="text-sm text-gray-600 mb-3">
          Argus monitors Claude Code and GitHub Copilot sessions on your machine and posts a message
          thread for each session. Updates are posted into the same thread as the session
          progresses.
        </p>
        <table className="w-full text-xs border-collapse">
          <thead>
          ...
          <tbody>
            {[
              [
                'Session started',
                'Repo, branch, tool (Claude Code / Copilot), mode, model, PID, session ID',
              ],
              ['Session updated', 'Only the fields that changed: status, model, Yolo mode, PID'],
              [
                'Action required',
                'Clickable choice buttons so you can respond from Teams or Slack',
              ],
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white border border-gray-200 rounded-lg px-6 py-5">
          ...
          <p className="text-sm text-gray-600 mb-3">
            Teams uses an inbound webhook model. Microsoft sends HTTPS POST requests to Argus, and
            Argus replies via the Bot Framework API.
          </p>
          ...
          <p className="text-sm text-gray-600 mt-3">
            Because Teams initiates the connection to Argus, a dev tunnel is required to make your
            local machine reachable. Use <Mono>devtunnel</Mono> to create a stable HTTPS URL that
            forwards to Argus on port 7411.
          </p>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg px-6 py-5">
          ...
          <p className="text-sm text-gray-600 mb-3">
            Slack uses an outbound model via Socket Mode. Argus opens a WebSocket connection to
            Slack — no public endpoint required.
          </p>
          <div className="bg-gray-50 border border-gray-200 rounded-md px-3 py-2.5 font-mono text-xs text-gray-700 space-y-1">
          ...
          <p className="text-sm text-gray-600 mt-3">
            Argus uses the <strong>Bot Token</strong> (<Mono>xoxb-</Mono>) to post messages and the{' '}
            <strong>App Token</strong> (<Mono>xapp-</Mono>) to keep the Socket Mode tunnel open for
            receiving @mentions and direct messages. No port forwarding or tunnel required.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg px-6 py-5">
        ...
        <p className="text-sm text-gray-600 mb-3">
          When a session is waiting for input, Argus posts a message with the available choices. You
          can respond directly from Teams or Slack:
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
          <div>
            <p className="font-medium text-gray-800 mb-1">Teams</p>
            <p>
              Argus posts an Adaptive Card with clickable buttons. Tap a button to send your choice
          ...
          <div>
            ...
            <p>
              Reply in the session thread or send a DM to the bot. Supported commands:{' '}
              <Mono>sessions</Mono>, <Mono>status &lt;id&gt;</Mono>, <Mono>help</Mono>.
            </p>
          </div>
        </div>
      </div>
