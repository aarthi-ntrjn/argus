...
export default function SessionMetaRow({
  session,
  showLink = false,
  onKill,
  killPending = false,
  ...
  const isKillable =
    onKill && session.pid != null && session.status !== 'ended' && session.status !== 'completed';

  return (
    <div className="flex justify-between items-start">
      <div className="flex flex-wrap gap-2 items-center">
        <Badge
          colorClass={TYPE_COLORS[session.type] ?? 'bg-gray-100'}
          icon={<SessionTypeIcon type={session.type} size={13} />}
        >
          {session.type}
        </Badge>
      ...
      <div className="flex items-center gap-2 shrink-0">
        {isInactive(session, thresholdMs) ? (
          <Badge
            ...
            icon={<Moon size={10} aria-hidden="true" />}
          >
            resting
          </Badge>
        ) : (
        ...
        {session.launchMode === 'pty' ? (
          <Badge
            ...
            title="Started via argus launch: prompt injection enabled"
          >
            connected
          </Badge>
        ) : (
          <Badge
            ...
            title="Detected session: start with argus launch to enable prompts"
          >
            read-only
          </Badge>
        )}
        {session.yoloMode === true && (
          <Badge
            ...
            icon={<ShieldOff size={10} aria-hidden="true" />}
            title="Session launched with auto-approve (yolo mode)"
          >
            yolo
          </Badge>
        )}
