...
### Addendum: Bug — Sessions not shown after adding repository until page refresh

- [X] T116 Fix `frontend/src/hooks/useRepositoryManagement.ts` `handleFolderSubmit()`: after adding repositories, only `queryClient.invalidateQueries({ queryKey: ['repositories'] })` is called (line 91), but the `['sessions']` query is never invalidated. The DashboardPage's `useQuery(['sessions'])` serves stale cached data, so newly discovered sessions for the added repos don't appear until a manual page refresh triggers a refetch. Compare with `handleRemoveRepoById()` (lines 107-108) which correctly invalidates both `['repositories']` AND `['sessions']`. Fix: add `await queryClient.invalidateQueries({ queryKey: ['sessions'] });` immediately after the existing `['repositories']` invalidation on line 91. Add a regression test in `frontend/tests/unit/useRepositoryManagement.test.ts` that verifies `invalidateQueries` is called with `['sessions']` after a successful folder submit.
