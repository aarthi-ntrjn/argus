...
### Addendum: Bug — PTY launch creates duplicate detected session

- [ ] T029 ClaudeCodeDetector creates a second read-only session when a PTY session already exists for the same repo: `handleHookPayload` and `activateFoundSession` in `backend/src/services/claude-code-detector.ts` call `getSession(claudeSessionId)` — which returns null because the PTY session has a different UUID — then create a new detected session (launchMode=null). Fix: (1) modify `watchJsonlFile(claudeSessionId, repoPath, storeAsId?)` to accept an optional override for the storage session ID, so the JSONL file (`<claudeSessionId>.jsonl`) can be watched but its output stored under the PTY session ID; (2) in `handleHookPayload` and `activateFoundSession`, when `getSession(claudeSessionId)` returns null, call `getSessions({repositoryId, status:'active'})` and find any session with `launchMode==='pty'`; if found, call `watchJsonlFile(claudeSessionId, repoPath, ptySession.id)` and update the PTY session's `lastActivityAt` — do NOT create a new detected session; if no PTY session, proceed as before. Also add `getSessions` to the imports in `claude-code-detector.ts`.
