...
export function getDb(): Database.Database {
  if (!db) {
    ...
    if (process.env.VITEST && !process.env.ARGUS_DB_PATH) {
      throw new Error(
        '[database] ARGUS_DB_PATH must be set when running under Vitest to prevent writing to ~/.argus/argus.db',
      );
    }
    ...
    const repoCols = (db.pragma('table_info(repositories)') as Array<{ name: string }>).map(
      (c) => c.name,
    );
    ...
    const sessionCols = (db.pragma('table_info(sessions)') as Array<{ name: string }>).map(
      (c) => c.name,
    );
    if (!sessionCols.includes('model')) {
      db.exec('ALTER TABLE sessions ADD COLUMN model TEXT');
    }
    const outputCols = (db.pragma('table_info(session_output)') as Array<{ name: string }>).map(
      (c) => c.name,
    );
    ...
    const yoloColInfo = (
      db.pragma('table_info(sessions)') as Array<{ name: string; notnull: number }>
    ...
    const teamsCols = (db.pragma('table_info(teams_threads)') as Array<{ name: string }>).map(
      (c) => c.name,
    );
    if (!teamsCols.includes('tenant_id')) {
      db.exec("ALTER TABLE teams_threads ADD COLUMN tenant_id TEXT NOT NULL DEFAULT ''");
    }
    const slackThreadsCols = (
      db.pragma('table_info(slack_threads)') as Array<{ name: string }>
    ).map((c) => c.name);
    if (!slackThreadsCols.includes('workspace_id')) {
      db.exec("ALTER TABLE slack_threads ADD COLUMN workspace_id TEXT NOT NULL DEFAULT ''");
    }
    const controlCols = (db.pragma('table_info(control_actions)') as Array<{ name: string }>).map(
      (c) => c.name,
    );
...
export function updateRepositoryBranch(
  id: string,
  branch: string | null,
  ...
  if (remoteUrl !== undefined) {
    getDb()
      .prepare('UPDATE repositories SET branch = ?, remote_url = ? WHERE id = ?')
      .run(branch, remoteUrl, id);
  } else {
...
export function deleteRepository(id: string): void {
  ...
  const sessionIds = (
    db.prepare('SELECT id FROM sessions WHERE repository_id = ?').all(id) as Array<{ id: string }>
  ).map((r) => r.id);
  ...
  if (sessionIds.length > 0) {
    ...
    const caCount = (
      db
        .prepare(`SELECT COUNT(*) as n FROM control_actions WHERE session_id IN (${placeholders})`)
        .get(...sessionIds) as { n: number }
    ).n;
    const soCount = (
      db
        .prepare(`SELECT COUNT(*) as n FROM session_output WHERE session_id IN (${placeholders})`)
        .get(...sessionIds) as { n: number }
    ).n;
    logger.info(
      `[deleteRepository] control_actions to delete=${caCount}, session_output to delete=${soCount}`,
    );
  }

  const caResult = db
    .prepare(
      'DELETE FROM control_actions WHERE session_id IN (SELECT id FROM sessions WHERE repository_id = ?)',
    )
    .run(id);
  const soResult = db
    .prepare(
      'DELETE FROM session_output WHERE session_id IN (SELECT id FROM sessions WHERE repository_id = ?)',
    )
    .run(id);
  const ttResult = db
    .prepare(
      'DELETE FROM teams_threads WHERE session_id IN (SELECT id FROM sessions WHERE repository_id = ?)',
    )
    .run(id);
  const stResult = db
    .prepare(
      'DELETE FROM slack_threads WHERE session_id IN (SELECT id FROM sessions WHERE repository_id = ?)',
    )
    .run(id);
  logger.info(
    `[deleteRepository] deleted control_actions=${caResult.changes}, session_output=${soResult.changes}, teams_threads=${ttResult.changes}, slack_threads=${stResult.changes}`,
  );
  ...
  if (sessionIds.length > 0) {
    ...
    const ttRemaining = (
      db
        .prepare(`SELECT COUNT(*) as n FROM teams_threads WHERE session_id IN (${placeholders})`)
        .get(...sessionIds) as { n: number }
    ).n;
    const stRemaining = (
      db
        .prepare(`SELECT COUNT(*) as n FROM slack_threads WHERE session_id IN (${placeholders})`)
        .get(...sessionIds) as { n: number }
    ).n;
    logger.info(
      `[deleteRepository] remaining after cleanup: control_actions=${caRemaining}, session_output=${soRemaining}, teams_threads=${ttRemaining}, slack_threads=${stRemaining}`,
    );
  }
...
export function getSessions(filters: SessionFilters = {}): Session[] {
  ...
  return (
    getDb()
      .prepare(sql)
      .all(...params) as Array<
      Omit<Session, 'reconciled' | 'yoloMode'> & { reconciled: number; yoloMode: number | null }
    >
  ).map((r) => ({
    ...r,
    reconciled: r.reconciled === 1,
    yoloMode: r.yoloMode === null ? null : r.yoloMode === 1,
  }));
}
...
export function getSession(id: string): Session | undefined {
  ...
  return {
    ...
    yoloMode: row.yoloMode === null ? null : row.yoloMode === 1,
  };
}
...
export function updateSessionStatus(
  ...
  if (reconciled !== undefined) {
    ...
    getDb()
      .prepare('UPDATE sessions SET status = ?, ended_at = ?, last_activity_at = ? WHERE id = ?')
...
export function upsertSession(session: Session): void {
  ...
  `,
    ...
    .run(
      ...
      session.pid,
      session.hostPid ?? null,
      session.pidSource ?? null,
      session.status,
      session.startedAt,
      session.endedAt,
      ...
      session.yoloMode === null ? null : session.yoloMode ? 1 : 0,
      session.ptyLaunchId ?? null,
    );
}

export function getOutputForSession(
  sessionId: string,
  limit = 100,
  before?: string,
): SessionOutput[] {
  let sql =
    'SELECT id, session_id as sessionId, timestamp, type, content, tool_name as toolName, tool_call_id as toolCallId, role, sequence_number as sequenceNumber FROM session_output WHERE session_id = ?';
...
export function getMaxSequenceNumber(sessionId: string): number {
  const row = getDb()
    .prepare(
      'SELECT COALESCE(MAX(sequence_number), 0) AS maxSeq FROM session_output WHERE session_id = ?',
    )
...
export function updateControlAction(
  id: string,
  status: string,
  ...
  getDb()
    .prepare('UPDATE control_actions SET status = ?, completed_at = ?, result = ? WHERE id = ?')
...
export function getTodos(userId = 'default'): TodoItem[] {
  return (
    getDb()
      ...
      .all(userId) as Array<Omit<TodoItem, 'done'> & { done: number }>
...
export function upsertTeamsThread(thread: TeamsThread): void {
  ...
  `,
    ...
    .run(
      thread.id,
      thread.sessionId,
      thread.teamsThreadId,
      thread.teamsChannelId,
      thread.tenantId,
      thread.createdAt,
    );
}
...
export function getIntegrationEnabled(id: string): boolean | null {
  const row = getDb().prepare('SELECT enabled FROM integrations WHERE id = ?').get(id) as
    | { enabled: number }
    | undefined;
  if (!row) {
    return null;
...
export function setIntegrationEnabled(id: string, enabled: boolean): void {
  getDb()
    .prepare('INSERT OR REPLACE INTO integrations (id, enabled, updated_at) VALUES (?, ?, ?)')
    .run(id, enabled ? 1 : 0, new Date().toISOString());
}

export function getServerState(key: string): string | null {
  const row = getDb().prepare('SELECT value FROM server_state WHERE key = ?').get(key) as
    | { value: string }
    | undefined;
