...
export class PtyRegistry {
  ...
  registerPending(tempId: string, ws: WebSocket, repoPath: string, hostPid: number, pid: number | null = null): void {
    ...
    logger.log(`[PtyRegistry] registerPending tempId=${tempId} hostPid=${hostPid} pid=${pid} repoPath="${repoPath}" key="${key}"`);
  ...
  updatePendingPid(repoPath: string, pid: number): void {
    ...
    if (pending) {
      logger.log(`[PtyRegistry] updatePendingPid tempId=${pending.tempId} pid=${pid} repoPath="${repoPath}"`);
      pending.pid = pid;
    }
  }
  ...
  claimByTempId(tempId: string, sessionId: string): { pid: number | null; hostPid: number } | null {
    for (const [key, pending] of this.pendingByRepoPath) {
      if (pending.tempId === tempId) {
        logger.log(`[PtyRegistry] claimByTempId tempId=${tempId} sessionId=${sessionId} hostPid=${pending.hostPid} pid=${pending.pid} key="${key}"`);
    ...
    logger.log(`[PtyRegistry] claimByTempId MISS tempId=${tempId} sessionId=${sessionId} — no pending entry found`);
    return null;
  }
  ...
  claimForSession(claudeSessionId: string, repoPath: string): { pid: number | null; hostPid: number } | null {
    ...
    if (!pending) {
      logger.log(`[PtyRegistry] claimForSession MISS sessionId=${claudeSessionId} repoPath="${repoPath}" key="${key}"`);
      return null;
    }
    logger.log(`[PtyRegistry] claimForSession OK sessionId=${claudeSessionId} hostPid=${pending.hostPid} pid=${pending.pid} repoPath="${repoPath}"`);
  ...
  updateClaimedPid(sessionId: string, pid: number): void {
    logger.log(`[PtyRegistry] updateClaimedPid sessionId=${sessionId} pid=${pid}`);
  ...
  unregisterPending(repoPath: string, tempId: string): void {
    logger.log(`[PtyRegistry] unregisterPending tempId=${tempId} repoPath="${repoPath}"`);
    this.pendingByRepoPath.delete(normalizePath(repoPath));
    this.tempToClaimedId.delete(tempId);
  }

  unregister(sessionId: string): void {
    logger.log(`[PtyRegistry] unregister sessionId=${sessionId}`);
  ...
  sendPrompt(sessionId: string, actionId: string, prompt: string, timeoutMs = 30_000): Promise<void> {
    ...
    logger.log(`[PtyRegistry.sendPrompt] sessionId=${sessionId} wsFound=${!!ws} wsState=${ws?.readyState}`);
    if (!ws) {
    ...
    return new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        logger.log(`[PtyRegistry.sendPrompt] TIMEOUT — no ack received within ${timeoutMs}ms actionId=${actionId} sessionId=${sessionId}`);
      ...
      logger.log(`[PtyRegistry.sendPrompt] sending WS message actionId=${actionId} promptLen=${prompt.length}`);
      ws.send(msg);
    });
  }
  ...
  handleAck(actionId: string, success: boolean, error?: string): void {
    ...
    logger.log(`[PtyRegistry.handleAck] actionId=${actionId} success=${success} found=${!!entry} error=${error ?? ''}`);
    if (!entry) return;
