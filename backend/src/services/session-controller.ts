...
export class SessionController {
  async stopSession(sessionId: string): Promise<ControlAction> {
    logger.log(`[stopSession] requested sessionId=${sessionId}`);
    const session = getSession(sessionId);
    if (!session) {
      logger.log(`[stopSession] NOT_FOUND sessionId=${sessionId}`);
      throw Object.assign(new Error(`Session ${sessionId} not found`), { code: 'NOT_FOUND' });
    }
    if (session.status === 'ended' || session.status === 'completed') {
      logger.log(`[stopSession] CONFLICT sessionId=${sessionId} status=${session.status}`);
      throw Object.assign(new Error('Session already ended'), { code: 'CONFLICT' });
    }
    if (!session.pid) {
      logger.log(`[stopSession] PID_NOT_SET sessionId=${sessionId}`);
      throw Object.assign(new Error('Session has no PID on record'), { code: 'PID_NOT_SET' });
    }

    logger.log(`[stopSession] validating PID ownership sessionId=${sessionId} pid=${session.pid} type=${session.type}`);
    ...
    if (!validation.valid) {
      ...
      logger.log(`[stopSession] validation failed sessionId=${sessionId} pid=${session.pid} reason=${validation.reason}`);
      throw Object.assign(new Error(message), { code });
    }

    logger.log(`[stopSession] validation passed, killing pid=${session.pid} sessionId=${sessionId}`);
    ...
    try {
      ...
      logger.log(`[stopSession] COMPLETED actionId=${action.id} sessionId=${sessionId} pid=${session.pid}`);
      return completed;
    } catch (err) {
      ...
      logger.log(`[stopSession] FAILED actionId=${action.id} sessionId=${sessionId} pid=${session.pid} error=${String(err)}`);
      return failed;
    }
  }
  ...
  async sendPrompt(sessionId: string, prompt: string): Promise<ControlAction> {
    ...
    logger.log(`[sendPrompt] sessionId=${sessionId} type=${session.type} launchMode=${session.launchMode} ptyRegistryHas=${ptyRegistry.has(sessionId)}`);
    ...
    ptyRegistry.sendPrompt(sessionId, action.id, prompt)
      .then(() => {
        ...
        logger.log(`[sendPrompt] DELIVERED actionId=${action.id} sessionId=${sessionId}`);
        updateControlAction(action.id, 'completed', now, null);
        this.broadcastAction({ ...action, status: 'completed', completedAt: now });
      })
      .catch((err: Error) => {
        const now = new Date().toISOString();
        logger.log(`[sendPrompt] FAILED actionId=${action.id} sessionId=${sessionId} error=${err.message}`);
