...
export class TeamsNotifier implements NotificationIntegration {
      ...
      this.log.info(
        {
          ...
          hasOwner: Boolean(config.ownerSenderId),
        },
        ...
        async () => {
          try {
            ...
            await this.teamsApp.api.conversations
              .activities(threadConvId)
              .create(this._buildReconnectMessage(session));
            this.log.info(
              `teams.thread.reused.notified: session=${session.id} teamsThreadId=${existing.teamsThreadId}`,
            );
          } catch (err) {
            if (isTeamsThreadNotFound(err)) {
              ...
              try {
                const sent = await this.teamsApp.send(
                  channelId,
                  this._buildOpeningMessage(session, repo),
                );
                upsertTeamsThread({
                  id: randomUUID(),
                ...
                this.log.error(
                  `teams.thread.stale.recover.failed: session=${session.id}`,
                  retryErr,
                );
              }
            } else {
    ...
    this.queue.enqueue(
      async () => {
        ...
        try {
          ...
          await this.teamsApp.api.conversations
            .activities(threadConvId)
            .create(new MessageActivity(text));
  ...
  async onSessionEnded(session: Session): Promise<void> {
    ...
    const thread = getTeamsThread(session.id);
    this.log.info(
      `teams.session.ended.thread-lookup: session=${session.id} threadFound=${!!thread} teamsThreadId=${thread?.teamsThreadId}`,
    );
    if (!thread) {
  ...
  _buildPendingChoiceMessage(choice: PendingChoice): MessageActivity {
    ...
    return new MessageActivity(fallbackText).addCard('adaptive', card);
  }
