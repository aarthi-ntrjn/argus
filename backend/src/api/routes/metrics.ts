...
const metricsRoutes: FastifyPluginAsync = async (app) => {
  app.get(
    ...
    async (_req, reply) => {
      const db = getDb();
      const activeRow = db
        .prepare("SELECT COUNT(*) as count FROM sessions WHERE status IN ('active','waiting')")
        .get() as { count: number };
      const endedRow = db
        .prepare("SELECT COUNT(*) as count FROM sessions WHERE status IN ('ended','completed')")
        .get() as { count: number };
      const outputRow = db.prepare('SELECT COUNT(*) as count FROM session_output').get() as {
        count: number;
      };
      const actionsRow = db.prepare('SELECT COUNT(*) as count FROM control_actions').get() as {
        count: number;
      };
