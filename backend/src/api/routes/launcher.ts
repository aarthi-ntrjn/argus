    ...
    (socket: WebSocket, request: FastifyRequest<{ Querystring: { id: string } }>) => {
      ...
      socket.on('message', (raw: Buffer) => {
        ...
        if (msg.type === 'register') {
          ...
          fastify.log.info(
            { ptyLaunchId, hostPid: msg.hostPid, pid: msg.pid, cwd: msg.cwd },
            '[Launcher] pending',
          );
          ...
          if (msg.pid !== null) {
            ...
            if (linkedId) {
              fastify.log.info(
                { ptyLaunchId, pid: msg.pid, sessionType: msg.sessionType, sessionId: linkedId },
                `[Launcher] ${msg.sessionType === 'claude-code' ? 'Claude Code' : 'Copilot CLI'}: PID resolved at register — pre-linked to sessionId`,
              );
            } else {
              fastify.log.info(
                { ptyLaunchId, pid: msg.pid, sessionType: msg.sessionType },
                `[Launcher] ${msg.sessionType === 'claude-code' ? 'Claude Code' : 'Copilot CLI'}: PID resolved at register — session file not yet written, will retry at claim time`,
              );
            }
          }
          ...
          if (existingSession) {
            ...
            if (livePid !== null && isPidRunning(livePid)) {
              ...
              fastify.log.info(
                { ptyLaunchId, sessionId: existingSession.id, priorStatus: existingSession.status },
                '[Launcher] reconnected to existing session via ptyLaunchId',
              );
              const now = new Date().toISOString();
              if (existingSession.status === 'ended') {
                ...
                broadcast({
                  ...
                  timestamp: now,
                  data: { ...restored, ptyConnected: true },
                });
                fastify.log.info(
                  { ptyLaunchId, sessionId: existingSession.id },
                  '[Launcher] restored session to active after server restart',
                );
              } else {
                broadcast({
                  type: 'session.updated',
                  timestamp: now,
                  data: { ...existingSession, ptyConnected: true },
                });
              }
            }
          }
        ...
        if (msg.type === 'update_pid') {
          ...
          if (ptyLaunchId) {
            ...
            if (sessionType) {
              ...
              if (linkedId) {
                fastify.log.info(
                  { ptyLaunchId, pid: msg.pid, sessionType, sessionId: linkedId },
                  `[Launcher] ${sessionType === 'claude-code' ? 'Claude Code' : 'Copilot CLI'}: update_pid resolved — pre-linked to sessionId`,
                );
              } else {
                fastify.log.info(
                  { ptyLaunchId, pid: msg.pid, sessionType },
                  `[Launcher] ${sessionType === 'claude-code' ? 'Claude Code' : 'Copilot CLI'}: update_pid resolved — session file not yet written, repoPath fallback will apply`,
                );
              }
            }
          }
          ...
          if (claudeSessionId) {
            ...
            if (session) {
              const yoloMode =
                session.yoloMode ||
                detectYoloModeFromPids(msg.pid, session.hostPid ?? null, session.type);
              ...
              fastify.log.info(
                { claudeSessionId, pid: msg.pid, yoloMode },
                '[Launcher] updated session with resolved tool PID',
              );
            } else {
              ...
              fastify.log.info(
                { claudeSessionId, pid: msg.pid },
                '[Launcher] parked resolved pid, session not yet in DB',
              );
            }
          }
          return;
        ...
        if (msg.type === 'session_ended') {
          ...
          if (claudeSessionId) {
            ...
            fastify.log.info(
              { claudeSessionId, exitCode: msg.exitCode },
          ...
          if (session && session.status !== 'ended') {
            ...
            if (livePid !== null && isPidRunning(livePid)) {
              ...
              broadcast({
                type: 'session.updated',
                timestamp: now,
                data: { ...session, ptyConnected: false },
              });
              fastify.log.info(
                { claudeSessionId, code },
                '[Launcher] disconnected but process alive, marked connecting',
              );
            } else {
              updateSessionStatus(claudeSessionId, 'ended', now);
              ...
              fastify.log.info(
                { claudeSessionId, code },
                '[Launcher] disconnected, session marked ended',
              );
            }
          } else {
            fastify.log.info(
              { claudeSessionId, status: session?.status, code },
              '[Launcher] disconnected, session already ended, no status change',
            );
          }
        } else if (repoPath) {
