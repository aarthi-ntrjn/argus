...
## Step 3: Get your Team ID, Channel ID, and Owner Sender ID

**Team ID:** Right-click the team name in Teams > **Get link to team**. The `groupId` parameter is your **Team ID**.

**Channel ID:** Right-click the channel > **Get link to channel**. The decoded value starting with `19:` is your **Channel ID** (format: `19:xxxxxxxx@thread.tacv2`).

**Owner Sender ID:** Open [graph.microsoft.com/v1.0/me](https://graph.microsoft.com/v1.0/me) in your browser, sign in with your Microsoft account, and copy the `id` field. This restricts inbound command access to you only.

## Step 4: Configure Argus

Open the Argus Settings dialog and go to the **Microsoft Teams** section. Enter the values from the previous steps:

- **Team ID** and **Channel ID**: from Step 3
- **Client ID** and **Tenant ID**: generated by `teams app create` in Step 2
- **Owner Sender ID**: from Step 3

Click **Save**. The webhook URL Argus listens on is shown at the bottom of the Teams settings panel: `{your-host}/api/v1/teams/webhook`.
...
| Symptom | Fix |
| ------- | --- |
| `teams: not configured, skipping event subscriptions` | Check all fields are saved in Settings and the integration is enabled. Restart Argus. |
| Messages not appearing | Confirm the bot is installed in the target team. Check Team ID and Channel ID are correct. |
...
| `401 Unauthorized` | Client ID or Client Secret is wrong. Re-run `teams app create` or check the Azure App Registration. |
