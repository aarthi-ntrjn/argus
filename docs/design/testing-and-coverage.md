...
| **Source scope** | `backend/src` — services, detectors, API routes, WebSocket handlers | `frontend/src` — React components, hooks, query logic | `frontend/src` — UI flows with all API calls mocked | `backend/src` — full stack with real Fastify server and SQLite DB |
