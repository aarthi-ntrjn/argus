...
Color coding: 🟢 ≥ 80%, 🟡 60–79%, 🔴 < 60%. Tests column: 🟢 = all passed, 🔴 = any failed. Skipped tests are excluded from the pass/total denominator and shown as `(N skipped)`.

Each suite also gets a full per-file breakdown section below the summary table.
