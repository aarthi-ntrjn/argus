...
| L0-07 | In the Claude terminal, type **"list the files in this repo"** directly (not from the dashboard) | The question and Claude's response appear in the session card's output stream on the dashboard within a few seconds |
| L0-08 | Click **View details** on the session card to open the session detail page; type **"what branch am I on?"** and send | Prompt is delivered via PTY; response appears in the output stream; input clears |
...
| L0-10 | On the second session card, type **"how many files are in this repo?"** in the prompt input and press Enter | The prompt appears in the second Claude terminal; Claude responds; the response is visible in both the terminal and the second session card output stream; the first session card is unaffected |
