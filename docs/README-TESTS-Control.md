...
| L0-10 | On the second session card, type **"what color is the sky?"** in the prompt input and press Enter | The prompt appears in the second Claude terminal; Claude responds; the response is visible in both the terminal and the second session card output stream; the first session card is unaffected |
